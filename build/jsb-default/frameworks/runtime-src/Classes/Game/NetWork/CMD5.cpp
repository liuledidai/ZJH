...
void CMD5::EncryptData( char *pszSrcData,char szMD5Result[33] )
{
	CMD5 MD5Encrypt;
	unsigned char szResult[16];
	MD5Encrypt.MD5Update((unsigned char *)pszSrcData,strlen((char*)pszSrcData)*sizeof(char));
	MD5Encrypt.MD5Final(szResult);

	//输出结果
	szMD5Result[0]=0;
	for (int i=0;i<16;i++) sprintf((char*)&szMD5Result[i*2],"%02x",szResult[i]);
